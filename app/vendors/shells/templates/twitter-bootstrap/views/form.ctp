<?php
echo "<?php \$errors = \$this->Form->validationErrors['{$modelClass}'];?>\n"
?>
<div class="row">
...
<?php
		foreach ($fields as $field) {
			if (strpos($action, 'add') !== false && $field == $primaryKey) {
				...
				?>
		<div class="clearfix<?php echo "<?php if (!empty(\$errors) && array_key_exists('{$field}',\$errors)) { echo ' error'; }?>";?>"<?php if ($field == $primaryKey) { ?> style="display:none;"<?php } ?>>
		<?php echo "\t<?php\n"; ?>
		<?php echo "\techo \$this->Form->label('{$field}');\n"; ?>
        <?php echo "\t\$after = '';\n"; ?>
        <?php echo "\tif (!empty(\$errors) && array_key_exists('{$field}',\$errors)) {\n"; ?>
        <?php echo "\t\t\$after = '<span class=\"help-inline\">'.\$errors['{$field}'].'</span>';\n"; ?>
        <?php echo "\t}\n"; ?>
        <?php echo "\techo \$this->Form->input('{$field}',array('after'=>\$after));\n"; ?>
        <?php echo "\t?>\n"; ?>
        </div>
<?php
			}
		...
		if (!empty($associations['hasAndBelongsToMany'])) {
			foreach ($associations['hasAndBelongsToMany'] as $assocName => $assocData) {
				?>
		<div class="clearfix<?php echo "<?php if (!empty(\$errors) && array_key_exists('{$assocName}',\$errors)) { echo ' error'; }?>";?>">
		<?php echo "\t<?php\n"; ?>
        <?php echo "\techo \$this->Form->label('{$assocName}');\n"; ?>
        <?php echo "\t\$after = '';\n"; ?>
        <?php echo "\tif (!empty(\$errors) && array_key_exists('{$assocName}',\$errors)) {\n"; ?>
        <?php echo "\t\t\$after = '<span class=\"help-inline\">'.\$errors['{$assocName}'].'</span>';\n"; ?>
        <?php echo "\t}\n"; ?>
        <?php echo "\techo \$this->Form->input('{$assocName}',array('after'=>\$after));\n"; ?>
        <?php echo "\t?>\n"; ?>
        </div>
<?php
			}
